  ...
  const updateRepairStatus = async (repairId, status) => {
    try {
      ...
      setRepairs(
        repairs.map((repair) =>
          repair.id === repairId
            ? { ...repair, status, technicianNote: note }
            : repair
        )
      );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {repairs.map((repair) => (
                ...
                >
                  <div className="col-span-2">
                    REP-{repair.id.slice(0, 6).toUpperCase()}
                  </div>
            ...
            <div className="bg-white rounded-lg shadow p-6 h-fit sticky top-6">
              <h2 className="text-lg font-bold mb-4">
                {selectedRepair
                  ? `จัดการคำขอ REP-${selectedRepair.id
                      .slice(0, 6)
                      .toUpperCase()}`
                  : "เลือกรายการซ่อม"}
              </h2>
              ...
              {selectedRepair && (
                <>
                  ...
                  <div className="space-y-2">
                    <button
                      onClick={() =>
                        updateRepairStatus(selectedRepair.id, "in_progress")
                      }
                    ...
                    <button
                      onClick={() =>
                        updateRepairStatus(selectedRepair.id, "completed")
                      }
                      className={`w-full py-2 px-4 rounded-md text-sm font-medium ${
                    ...
                    <button
                      onClick={() =>
                        updateRepairStatus(selectedRepair.id, "rejected")
                      }
                      className={`w-full py-2 px-4 rounded-md text-sm font-medium ${
